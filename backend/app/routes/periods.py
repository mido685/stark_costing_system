...
def validate_period(
    ...
    """
    Dry-run all pre-close checks without closing.
    Returns 422 with the blocking reason if any check fails.
    """
    try:
...
# ─── GET /api/period/snapshot?period=YYYY-MM ─────────────────────────────────
...
def get_snapshot(
    ...
    """
    Return the frozen financial snapshot for a locked period.
    Returns 404 if the period has not been hard-locked yet.
    """
    company_id = current_user["company_id"]
    snapshot = get_period_snapshot(company_id, period)
    if not snapshot:
        return error(
            f"No snapshot for period {period}. Snapshots are only created on hard lock.",
            status=404,
        )
    return success("Period snapshot fetched", **snapshot)


# ─── POST /api/period/adjusting-entry ────────────────────────────────────────

class AdjustingEntryBody(BaseModel):
    branch_id:         int
    amount:            float
    reason:            str
    references_period: str   # "YYYY-MM" — must already be locked


@router.post("/adjusting-entry")
def create_adjusting_entry(
    body: AdjustingEntryBody,
    current_user: dict = Depends(require_roles("owner", "admin")),
):
    """
    Post a correcting entry in the current open period that references
    a past locked period. The locked period and its snapshot are never touched.
    Only Owner/Admin can post adjusting entries (mirrors lock permissions).
    """
    try:
        row = post_adjusting_entry(
            company_id=current_user["company_id"],
            branch_id=body.branch_id,
            amount=body.amount,
            reason=body.reason,
            user_id=current_user["id"],
            references_period=body.references_period,
        )
        return success("Adjusting entry posted", **row)
    except ValueError as e:
        return error(str(e), status=422)
