// src/components/SystemLogsTab.tsx
import { useEffect, useState, useCallback, type ReactNode } from "react";
import { Card } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import {
  Activity, AlertCircle, AlertTriangle, ChevronDown,
...
const LEVEL_CONFIG: Record<string, { icon: ReactNode; badge: string; dot: string }> = {
  ...
  critical: { icon: <XCircle className="w-3.5 h-3.5" />,       badge: "bg-red-200 dark:bg-red-900/60 text-red-800 dark:text-red-200 border-red-300 dark:border-red-700",             dot: "bg-red-700"    },
};
    ...
    <>
      <tr className={`border-b border-border transition-colors ${expanded ? "bg-secondary/20" : "hover:bg-secondary/30"}`}>
        <td className="px-4 py-3 whitespace-nowrap">
          <span className={`inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-md font-semibold ${actionBadgeClass(log.action)}`}>
            {log.action}
          </span>
        </td>
        <td className="px-4 py-3 whitespace-nowrap">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <Button size="sm" variant="outline" onClick={() => fetchLogs(filters, offset)} disabled={loading} className="gap-2">
