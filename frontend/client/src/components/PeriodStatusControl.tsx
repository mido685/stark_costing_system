 ...
 *
 * Design: Refined dark-glass aesthetic with micro-animations
 * Fixes:
 *  1. Panel never clips — action buttons always visible, no overflow-hidden on outer
 *  2. Inner zones scroll independently with proper max-h
 *  3. Staggered entrance animations on panel open
 *  4. Animated status badge with pulsing dot
 *  5. Smooth tab underline via CSS translate
 *  6. Action buttons have hover lift + press feedback
 *  7. Calendar cells animate on hover
 *  8. Toast slides in from top-right
 *  9. All transitions use cubic-bezier for premium feel
 */
...
const css = `
  ...
  .psc-tab-indicator {
    ...
    border-radius: 2px; transition: left 0.2s cubic-bezier(0.4,0,0.2,1), width 0.2s cubic-bezier(0.4,0,0.2,1);
  }
`;
  ...
  return (
    <div className="py-1">
      {entries.map((e, i) => (
        <div
          key={i}
          className="psc-row flex gap-3 px-3.5 py-2.5 relative group"
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${STATUS[e.from_status].pill}`}>
                {STATUS[e.from_status].label}
              </span>
              <span className="text-[9px] text-muted-foreground">→</span>
              <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${STATUS[e.to_status].pill}`}>
                {STATUS[e.to_status].label}
              </span>
            </div>
...
function ActionBtn({
  icon, label, desc, danger = false, disabled = false, onClick, loading = false,
}: {
  ...
  return (
    ...
    >
      <span className={`shrink-0 w-7 h-7 rounded-lg flex items-center justify-center ${
        danger ? "bg-red-500/10 text-red-400" : "bg-muted text-muted-foreground"
      }`}>
        {loading ? <Spinner /> : icon}
      </span>
      <div className="flex-1 min-w-0">
        <p className={`text-[12px] font-semibold ${danger ? "text-red-400" : "text-foreground"}`}>
          {label}
        </p>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-muted-foreground mt-0.5">
            Correction posted in current open period · references {fmtShortPeriod(period)}
          </p>
        </div>
    ...
    <>
      ...
      <div className="relative">
        ...
        {toast && (
          <div
            className={`
              psc-toast absolute right-0 -top-11 z-[70] flex items-center gap-2
        ...
        {open && (
          <div
            ref={panelRef}
            className="psc-panel absolute right-0 top-full mt-2 z-50 w-[300px] rounded-2xl border border-border/60 bg-card shadow-2xl"
            style={{ boxShadow: "0 20px 60px -10px rgba(0,0,0,0.4), 0 0 0 1px rgba(255,255,255,0.04)" }}
          >
            {drillPeriod ? (
              /* ── Drill view ── */
              <>
                <div className="flex items-center gap-2.5 px-4 py-3 border-b border-border/50">
                  ...
                  <div>
                    <p className="text-[13px] font-bold text-foreground">
                      {fmtShortPeriod(drillPeriod.period)}
                    </p>
                  ...
                  <div className="ml-auto">
                    <StatusPill status={drillPeriod.status} />
                  </div>
                </div>
              ...
              <>
                ...
                {tab === "actions" && (
                  <div>
                    ...
                    {/* Adjusting entry form OR action buttons */}
                    {showAdjForm && status === "locked" ? (
                      ...
                      /* Action buttons — always scrollable so nothing clips */
                      <div className="p-2.5 flex flex-col gap-1.5 max-h-52 overflow-y-auto">
                        {status === "open" && (
                          <>
                            <ActionBtn
                              ...
                              desc="Validate data integrity before closing"
                              loading={acting}
                            ...
                            {canClose && (
                              <ActionBtn
                                ...
                                label="Soft Close Period"
                                desc="Block writes, keep data reviewable"
                                loading={acting}
                        ...
                        {status === "closed" && (
                          <>
                            ...
                            <ActionBtn
                              icon={<Eye size={14} />}
                              label="View Transition History"
                              desc="Audit all status changes"
                            ...
                            {canLock && (
                              <>
                                ...
                                <ActionBtn
                                  icon={<Lock size={14} />}
                                  label="Hard Lock Period"
                                  desc="Freeze permanently · irreversible"
                                  danger
                                  loading={acting}
                        ...
                        {status === "locked" && (
                          <>
                            <ActionBtn
                              icon={<Eye size={14} />}
                              label="View Frozen Snapshot"
                              desc="Figures captured at lock time"
                              onClick={() => setTab("history")}
                            />
                            <ActionBtn
                              icon={<SlidersHorizontal size={14} />}
                              label="Post Adjusting Entry"
                              desc="Correction in current open period"
                              onClick={() => setShowAdjForm(true)}
                            />
                            <ActionBtn
                              icon={<Lock size={14} />}
                              label="Re-open Period"
                              desc="Not permitted on hard-locked periods"
                              disabled
                              onClick={() => {}}
                            />
                          </>
