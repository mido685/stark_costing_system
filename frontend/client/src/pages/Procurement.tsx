   ...
   * Tabs: Standard POs · Cash Purchases · Petty Cash · Invoices
   * Fixes: Blob re-wrap corruption → blob.slice() for correct MIME typing
   */
  ...
  type TabKey          = "po" | "cash" | "petty" | "invoices";
  ...
  const initInvoiceForm = (): InvoiceForm => ({
  ref_table: "cash_purchases", ref_id: 0, notes: "", file: null,
  branch_id: undefined, supplier_id: undefined,
  invoice_number: "", invoice_date: "", amount: undefined,
  });
  ...
  export default function Procurement() {
    ...
    const handleDownloadPoPdf = useCallback(async (id:number) => {
      ...
      try {
        ...
        // ✅ Fix: use slice() to re-type without re-wrapping
        const typedBlob = blob.slice(0, blob.size, "application/pdf");
        const url = URL.createObjectURL(typedBlob);
        const a   = document.createElement("a");
        a.href = url; a.download = `PO-${id}.pdf`;
        document.body.appendChild(a); a.click(); document.body.removeChild(a);
        setTimeout(() => URL.revokeObjectURL(url), 1000);
      } catch (err) { console.error("PO PDF download failed:", err); }
    ...
    // ── Petty cash (declared before handleApproveCash) ─────────────────────────
    ...
    async function handleUploadInvoice() {
      ...
      try {
        ...
        setModal(null);
        await handleSearchInvoices();  // ← auto-refresh search after upload
    ...
    // Procurement.tsx — handleInvoiceDownload
    async function handleInvoiceDownload(inv: Invoice) {
      try {
        const token = localStorage.getItem("token") ?? "";
        const resp  = await fetch(`/api/invoices/file/${inv.id}`, {  // ← was /download/
          headers: { Authorization: `Bearer ${token}` },
        });
        if (!resp.ok) throw new Error(`Server returned ${resp.status}`);
        const blob      = await resp.blob();
                ...
                <TableWrap>
                  <thead>
                    <tr className="border-b border-border">
                      ...
                      <Th center>Status</Th><Th center>PDF</Th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border/60">
                    {filteredPurchases.slice(0,50).map((row,i) => (
                      <tr key={row.id??i} className="hover:bg-muted/30 transition-colors">
                        ...
                        <Td center>
                          <button onClick={()=>handleDownloadPoPdf(row.id)} disabled={downloadingId===row.id}
                            aria-label={`Download PDF for PO #${row.id}`}
                            ...
                            {downloadingId===row.id ? <Loader2 className="w-3.5 h-3.5 animate-spin"/> : <FileDown className="w-3.5 h-3.5"/>}
                          </button>
                        </Td>
                      </tr>
                    ))}
              ...
              <Card className="p-6 border border-border/60">
                ...
                {invoicesLoading ? (
                  ...
                  <EmptyState
                    ...
                    cta={
                      <Button size="sm" variant="outline"
                        onClick={() => openModal("invoice_upload")} className="gap-1.5">
                        <Upload className="w-4 h-4"/> Upload Invoice
                      </Button>
                    }
                  />
